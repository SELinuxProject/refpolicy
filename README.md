# SELinux Reference Policy

The SELinux Reference Policy project (refpolicy) is a complete SELinux
policy that can be used as the system policy for a variety of systems
and used as the basis for creating other policies.

The current release of Reference Policy is available from the
[GitHub releases](https://github.com/SELinuxProject/refpolicy/releases) page.

The project is always looking for policy developers interested in
contributing. If you'd like to contribute, please see
[how to contribute](doc/HOW_TO_CONTRIBUTE.md). See the
[getting started guide](doc/GETTING_STARTED.md) for more information on writing
Reference Policy modules.

For an in-depth discussion of Reference Policy concepts, see the
[paper](doc/WHITEPAPER.md) published at the 2006 SELinux Symposium.

The current project-wide objectives are defined in the
[security goals](doc/SECURITY_GOALS.md).
